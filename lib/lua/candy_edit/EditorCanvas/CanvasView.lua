...
local EditorCanvasCamera = EditorCanvasSceneModule.EditorCanvasCamera
local CanvasGrid = require 'candy_edit.EditorCanvas.CanvasGrid'
local CanvasNavigate = require 'candy_edit.EditorCanvas.CanvasNavigate'
...
local PickingManager = require 'candy_edit.EditorCanvas.PickingManager'


---@class CanvasView : EditorEntity
local CanvasView = CLASS:CanvasView  ( EditorEntity )
...
function CanvasView:createCamera ( canvasEnv )
	local cameraEntity = candy.EditorEntity ()
...
function CanvasView:initContext ()
	...
	local inputDevice = EditorCanvasSceneModule.createEditorCanvasInputDevice ( self.canvasEnv )
	self:attach ( candy.InputScript { device = inputDevice } )
...
function CanvasView:initAddons ()
	...
	self.toolManager    = self:addChild ( CanvasToolModule.CanvasToolManager () )
	self.gizmoManager   = self:addChild ( GizmoManagerModule.GizmoManager () )
	self.itemManager    = self:addChild ( CanvasItemModule.CanvasItemManager () )
...
function CanvasView:pickAndSelect ( x, y, pad )
	...
	candy.changeSelection ( 'scene', unpack ( picked ) )
	return picked
end
