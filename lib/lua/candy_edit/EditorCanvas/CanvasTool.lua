...
-- module
local CanvasToolModule = {}

-- class
--local CanvasToolManager
--local CanvasTool


local CanvasToolRegistry = {}
--------------------------------------------------------------------
local function registerCanvasTool ( id, clas )
	CanvasToolRegistry[ id ] = clas
end

--------------------------------------------------------------------
---@class CanvasToolManager : SceneEventListener
local CanvasToolManager = CLASS: CanvasToolManager ( SceneEventListener )
	:MODEL {}
...
local CanvasTool = CLASS: CanvasTool ( SceneEventListener )
...
function CanvasTool:getSelection ( key )
	return candy.getSelection ( key or 'scene' )
end
...
function CanvasTool:findTopLevelEntities ( entities )
	...
	for a in pairs ( entities ) do
		...
		if isTop then found[a] = true end
	end
	return found
end
...
CanvasToolModule.CanvasToolManager = CanvasToolManager
CanvasToolModule.CanvasTool = CanvasTool
CanvasToolModule.registerCanvasTool = registerCanvasTool

return CanvasToolModule