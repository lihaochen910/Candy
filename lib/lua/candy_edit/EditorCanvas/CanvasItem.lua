...
-- module
local CanvasItemModule = {}

---@class CanvasItemManager : EditorEntity
local CanvasItemManager = CLASS: CanvasItemManager ( candy.EditorEntity )
	:MODEL {}
...
function CanvasItemManager:onLoad ()
	...
	assert ( isInstance ( view, candy_edit.CanvasView ) )

	self.inputScript = self:attachInternal ( 
		candy.InputScript { device = view:getInputDevice () }
	)
...
local CanvasItem = CLASS: CanvasItem ( candy.EditorEntity )
	:MODEL {}
...
CanvasItemModule.CanvasItemManager = CanvasItemManager
CanvasItemModule.CanvasItem = CanvasItem

return CanvasItemModule