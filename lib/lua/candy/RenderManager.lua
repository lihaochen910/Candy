...
local _RenderManager = nil

---@return RenderManager
local function getRenderManager ()
	return _RenderManager
end

local createTableRenderLayer
local createTableViewRenderLayer
local createPartitionRenderLayer

--------------------------------------------------------------------
-- GlobalTextureItem
--------------------------------------------------------------------
---@class GlobalTextureItem
local GlobalTextureItem = CLASS: GlobalTextureItem ()
...
--------------------------------------------------------------------
---@class RenderManager
local RenderManager = CLASS: RenderManager ()
	:MODEL {}

function RenderManager:__init ( name )
	_RenderManager = self
	self.async = false --MOAIRenderMgr.isAsync ()
	self.name = name
	...
	self.rootRenderPass = createTableRenderLayer () ---@type MOAITableLayer
	...
	SignalModule.connectGlobalSignalFunc ( "gfx.pre_sync_render_state", function ()
		return self:preSyncRenderState ()
	end )
	SignalModule.connectGlobalSignalFunc ( "gfx.post_sync_render_state", function ()
...
function RenderManager:registerContext ( name, context )
	...
	context = context or candy.RenderContext ()
	context.name = name
...
function RenderManager:setCurrentContext ( context )
	if type ( context ) == "string" then
		...
		assert ( context, "RenderManager.contextMap has nil conext: " .. contextKey )
	end

	local context0 = self.currentContext
	if context0 == context then
		return
	end
...
function RenderManager:onContextReady ()
	...
	SignalModule.emitGlobalSignal ( "gfx.render_manager_ready" )
end
...
function RenderManager:declareGlobalTexture ( name )
	...
	if self.maxGlobalTextureCount < idx then
...
function RenderManager:getGlobalTextureUnit ( name )
...
function RenderManager:addPostSyncRenderCall ( func, ... )
	if self.async then
		table.insert ( self.pendingPostSyncCalls, {
			func,
			{ ... }
		} )
	else
...
function RenderManager:addRenderTask ( layer, callback, data )
	...
	if pending then
		table.insert ( self.pendingRenderTasks, {
			layer,
			callback,
			data
		} )
...
function RenderManager:setPerformanceProfile ( profile )
	...
	SignalModule.emitGlobalSignal ( "gfx.performance_profile.change", profile )
...
RenderManagerModule.getRenderManager = getRenderManager
RenderManagerModule.GlobalTextureItem = GlobalTextureItem
RenderManagerModule.RenderManager = RenderManager
RenderManagerModule.createTableRenderLayer = createTableRenderLayer
RenderManagerModule.createTableViewRenderLayer = createTableViewRenderLayer
RenderManagerModule.createPartitionRenderLayer = createPartitionRenderLayer

return RenderManagerModule