...
-- module
local JoystickManagerModule = {}

local _joystickManager = nil
...
local JoystickMapping = CLASS: JoystickMapping ()
	:MODEL {}
...
--------------------------------------------------------------------
---@class JoystickState
local JoystickState = CLASS: JoystickState ()
	:MODEL {}
...
function JoystickState:initButtonSensor()
	...
	device.buttons:setCallback(function (btn, down)
		if self._mgr then
...
function JoystickState:initAxisSensor()
	...
	for axisId = 0, instance:getAxeCount() - 1 do
		...
		axisSensor:setCallback(function (value)
			if self._mgr then
...
function JoystickState:initHatSensor()
	...
	for hatId = 0, instance:getHatCount() - 1 do
		...
		if hatSensor then
			hatSensor:setCallback(function (dir, down)
				if self._mgr then
...
function JoystickState:updateAxisArrowButton(axisId, v, pv)
	...
	local i0 = gate < pv and 1 or pv < -gate and -1 or 0
	local btnHigh, btnLow = nil
...
local JoystickManager = CLASS: JoystickManager ( GlobalManager )
	:MODEL {}
...
function JoystickManager:addJoystickListener(func)
	if index(self.joystickListeners, func) then
		return
	end
...
local DummyJoystickManager = CLASS: DummyJoystickManager ( JoystickManager )

local defaultBtnConfirm = "a"
local defaultBtnCancel = "b"

function setBasicUIJoystickButtons(btnConfirm, btnCancel)
	defaultBtnCancel = btnCancel
	defaultBtnConfirm = btnConfirm
end
...
JoystickManagerModule.getJoystickManager = getJoystickManager
JoystickManagerModule.JoystickMapping = JoystickMapping
JoystickManagerModule.JoystickState = JoystickState
JoystickManagerModule.JoystickManager = JoystickManager
JoystickManagerModule.DummyJoystickManager = DummyJoystickManager
JoystickManagerModule.setBasicUIJoystickButtons = setBasicUIJoystickButtons
JoystickManagerModule.isConfirmButton = isConfirmButton
JoystickManagerModule.isCancelButton = isCancelButton

return JoystickManagerModule