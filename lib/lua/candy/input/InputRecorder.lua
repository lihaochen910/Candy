...
local InputEventRecorder = CLASS: InputEventRecorder ()

function InputEventRecorder:__init ( option )
	self.thread = false
	self.recording = false
	option = option or {}
	self.filter = option.filter or {
		"mouse",
		"keyboard",
		"touch",
		"compass",
...
function InputEventRecorder:start ( recordtime, autosave )
	...
	local function _record ( tag )
		return function  ( ... )
			return self:pushEvent ( tag, ... )
		end
	end

	if recordtime then
		laterCall ( recordtime, function  ()
			self:stop ()
...
function InputEventRecorder:save ( filename )
	...
	if not f then
		error ( "cannot open file for save:" .. filename )
	end
...
local InputEventPlayer = CLASS: InputEventPlayer ()
...
function InputEventPlayer:play ( speed )
	...
	thread:run ( function ()
		...
		while true do
			if not self:playOneFrame ( speed ) then
				break
			end
...
function InputEventPlayer:playOneFrame ( speed )
	if self.dataSize < self.playPos then
		return false
	end
...
InputRecorderModule.InputEventRecorder = InputEventRecorder
InputRecorderModule.InputEventPlayer = InputEventPlayer
InputRecorderModule.playOrRecordInputEvent = playOrRecordInputEvent

return InputRecorderModule