-- import
local Event = require 'candy.common.Event'
local EventListener = require 'candy.common.EventListener'
local EntityModule = require 'candy.Entity'
local Entity = EntityModule.Entity

---@class EventDispatcherEntity : Entity
local EventDispatcherEntity = CLASS: EventDispatcherEntity ( Entity )
...
---
-- The constructor.
function EventDispatcherEntity:__init ()
...
function EventDispatcherEntity:addEventListener ( eventType, callback, source, priority )
    assert ( eventType )
    assert ( callback )
...
function EventDispatcherEntity:removeEventListener ( eventType, callback, source )
    assert ( eventType )
    assert ( callback )
...
function EventDispatcherEntity:setEventListener ( eventType, callback, source, priority )
...
function EventDispatcherEntity:hasEventListener ( eventType, callback, source )
    assert ( eventType )
...
function EventDispatcherEntity:dispatchEvent ( event, data )
...
function EventDispatcherEntity:clearEventListeners ()
    self.eventlistenersMap = {}
end

return EventDispatcherEntity