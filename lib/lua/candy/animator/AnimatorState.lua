...
local function _isFinite ( mode )
	if mode == MOAITimer.NORMAL then
		return true
	end

	if mode == MOAITimer.REVERSE then
		return true
	end
...
---@class AnimatorState
local AnimatorState = CLASS: AnimatorState ()
	:MODEL {}
...
function AnimatorState:__init ()
	...
	self.elapsedTimer:setListener ( MOAIAnim.EVENT_TIMER_END_SPAN, function  ()
		return self:stop ()
	end )
...
function AnimatorState:loadClip ( animator, clip )
	...
	for track in pairs ( context.playableTracks ) do
		if track:isLoadable ( self ) and  ( not previewing or track:isPreviewable () ) then
	...
	for i, linkInfo in ipairs ( self.attrLinks ) do
		...
		if target and  ( not previewing or track:isPreviewable () ) then
	...
	table.sort ( self.updateListenerTracks, function  ( lhs, rhs )
		local t1 = lhs[ 1 ]
		local t2 = rhs[ 1 ]
...
function AnimatorState:addAttrLink ( track, curve, target, id, asDelta )
	...
	self.attrLinks[ self.attrLinkCount ] = {
		track,
		curve,
		target,
		id,
