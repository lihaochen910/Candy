-- import
local FSMController = require 'candy.ai.FSMController'
...
local AnimatorFSM = CLASS: AnimatorFSM ( FSMController )
	:MODEL {
...
function AnimatorFSM:onStart ( ent )
	local animator = ent:com ( candy.Animator )
...
function AnimatorFSM:acceptStateChange ( state )
	...
	if optional == "reject" and name then
		local animator = self.animator
		if not animator or not animator:hasClip ( name ) then
			return false
		end
	end
...
function AnimatorFSM:setState ( state )
	...
	if state:endwith ( ".start" ) or state:endwith ( ".stop" ) then
		return
	end
	...
	if name then
		...
		local state = animator:loadClip ( name, true )
...
ComponentModule.registerComponent ( "AnimatorFSM", AnimatorFSM )
