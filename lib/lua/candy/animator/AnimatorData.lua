...
local AnimatorClipGroup = AnimatorClipModule.AnimatorClipGroup

local tinsert = table.insert

---@class AnimatorData
local AnimatorData = CLASS: AnimatorData ()
	:MODEL {
		Field ( "name" ):string (),
		"----",
...
function AnimatorData:sortEvents ()
	for i, clip in ipairs ( self.clips ) do
...
function AnimatorData:_postLoad ()
...
function loadAnimatorDataFromRaw ( data )
	...
	local animatorData = candy.deserialize ( nil, data )
...
function loadAnimatorData ( node )
	local data = candy.loadAssetDataTable ( node:getObjectFile ( "data" ) )
	node:disableGC ()
	return loadAnimatorDataFromRaw ( data )
end


AssetLibraryModule.registerAssetLoader ( "animator_data", loadAnimatorData )
