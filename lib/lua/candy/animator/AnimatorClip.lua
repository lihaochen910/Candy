-- module
local AnimatorClipModule = {}

local insert = table.insert

local AnimatorClipSubNode = CLASS: AnimatorClipSubNode ()
local AnimatorClipSubNodeSpan = CLASS: AnimatorClipSubNodeSpan ()
local AnimatorKey = CLASS: AnimatorKey ()
local AnimatorTrack = CLASS: AnimatorTrack ( AnimatorClipSubNode )
local AnimatorTrackGroup = CLASS: AnimatorTrackGroup ( AnimatorClipSubNode )
local AnimatorClipMarker = CLASS: AnimatorClipMarker ()
local AnimatorClip = CLASS: AnimatorClip ()
local AnimatorClipGroup = CLASS: AnimatorClipGroup ()
...
AnimatorClipSubNodeSpan
	:MODEL {
		Field ( "id" ):int (),
		Field ( "name" ):string (),
		Field ( "pos" ):float ():range ( 0 ):meta ( {
			step = 0.1
		} ),
		Field ( "length" ):float ():range ( 0 ):meta ( {
			step = 0.1
		} )
	}
...
AnimatorKey
	:MODEL {
		...
		Field ( "parentSpanId" ):int ():no_edit (),
		Field ( "pos" ):float ():range ( 0 ):getset ( "Pos" ):meta ( {
			decimals = 3,
			step = 0.01
		} ),
		Field ( "length" ):float ():range ( 0 ):getset ( "Length" ):meta ( {
			decimals = 3,
			step = 0.01
		} ),
		"----",
...
function AnimatorKey:findNextKey ( allowWrap )
	...
	for i, k in ipairs ( keys ) do
		...
		if e ~= self and pos0 < pos1 and  ( not pos or pos1 < pos ) then
			res = e
			pos = pos1
		end
	end
...
function AnimatorKey:getInfoText ()
	return false
end
...
function AnimatorTrack:getTargetObject ( rootEntity, scene )
	...
	if not rootEntity and not scene then
		return nil
	end
...
function AnimatorTrack:buildIdCurve ( addEmptySpan )
	...
	if not addEmptySpan then
		...
		for i, key in ipairs ( keys ) do
			local t0 = key:getPos ()
			local t1 = t0 + key:getLength ()

			if i > 1 and pt1 < t0 then
				insert ( ids, {
					pt1,
					0
				} )
...
local AnimatorClipBuildContext = CLASS: AnimatorClipBuildContext ()
...
local AnimatorRecordingState = CLASS: AnimatorRecordingState ()
...
function AnimatorRecordingState:markFieldRecording ( obj, fieldId )
	...
	if state[ fieldId ] then
		return
	end

	state[ fieldId ] = {
		model:getFieldValue ( obj, fieldId )
	}
end
...
AnimatorClipModule.AnimatorClipSubNode = AnimatorClipSubNode
AnimatorClipModule.AnimatorClipSubNodeSpan = AnimatorClipSubNode
AnimatorClipModule.AnimatorKey = AnimatorKey
AnimatorClipModule.AnimatorTrack = AnimatorTrack
AnimatorClipModule.AnimatorTrackGroup = AnimatorTrackGroup
AnimatorClipModule.AnimatorClipMarker = AnimatorClipMarker
AnimatorClipModule.AnimatorClip = AnimatorClip
AnimatorClipModule.AnimatorClipGroup = AnimatorClipGroup

return AnimatorClipModule