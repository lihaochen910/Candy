...
---@class Animator
local Animator = CLASS: Animator ( Component ) :MODEL ( {
	Field ( "data" ):asset_pre ( "animator_data" ):getset ( "DataPath" ),
	"----",
	Field ( "noPooling" ):boolean (),
	Field ( "throttle" ):number ():range ( 0 ):meta ( {
		step = 0.1
	} ):getset ( "Throttle" ),
	"----",
...
function Animator:setDataPath ( dataPath )
	...
	self.data = candy.loadAsset ( dataPath )
...
function Animator:loadClip ( clip, makeActive, _previewing )
	...
	elseif isInstance ( clip, AnimatorClip ) then
		clipData = clip
	end
...
function Animator:onStateStop ( state )
	if not state.previewing then
		...
		if state == self.activeState and self.retainedState then
...
ComponentModule.registerComponent ( "Animator", Animator )
