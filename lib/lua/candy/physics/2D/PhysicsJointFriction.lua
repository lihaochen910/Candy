...
local PhysicsJointFriction = CLASS: PhysicsJointFriction ( PhysicsJoint )
	:MODEL ( {
...
ComponentModule.registerComponent ( "PhysicsJointFriction", PhysicsJointFriction )
...
function PhysicsJointFriction:createJoint ( bodyA, bodyB )
	...
	local joint = self:getB2World ():addFrictionJoint ( bodyA, bodyB, x0, y0, self.maxForce, self.maxTorque )
	return joint
end
