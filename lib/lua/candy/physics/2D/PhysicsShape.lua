...
---@class PhysicsShape : PhysicsJoint
local PhysicsShape = CLASS: PhysicsShape ( Component )
	:MODEL {
...
function PhysicsShape:updateMaterial ()
	...
	shape:setFilter ( material.categoryBits or 1, material.maskBits or 65535, material.group or 0 )
	self.parentBody:updateMass ()
...
function PhysicsShape:setFilter ( categoryBits, maskBits, group )
	...
	local material = self:affirmMaterial ()
	shape:setSensor ( material.isSensor )
	shape:setFilter ( categoryBits or material.categoryBits or 1, maskBits or material.maskBits or 255, group or material.group or 0 )
...
function PhysicsShape:resetFilter ()
	...
	if sourceMaterial then
		self:setFilter ( sourceMaterial.categoryBits or 1, sourceMaterial.maskBits or 65535, sourceMaterial.group or 0 )
	end
end
...
function PhysicsShape:updateCollisionHandler ()
	...
	if not self.handlerData then
		return
	end

	self.shape:setCollisionHandler ( self.handlerData.func, self.handlerData.phaseMask, self.handlerData.categoryMask )
end
