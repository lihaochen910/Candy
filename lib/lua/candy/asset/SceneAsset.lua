...
SceneAssetModule._SERIALIZER_VERSION = '2'
SceneAssetModule._SCENE_INDEX_NAME = 'scene_index.json'
SceneAssetModule._SCENE_GROUP_EXTENSION = '.scene_group'
...
--------------------------------------------------------------------
local function _printActorId ( entity, i )
	i = i or 0
	print ( string.rep ( '\t', i ) .. entity.__guid .. ':' .. entity:getName () )
	...
	for child in pairs ( entity.children ) do
		_printActorId ( child, i + 1 )
	end
end

local function printSceneId ( scn )
	for entity in pairs ( scn.entities) do
		_printActorId ( entity, 0 )
	end
end

printActorId = _printActorId

--------------------------------------------------------------------
local function actorSortFunc ( a, b )
...
local SceneSerializer = CLASS: SceneSerializer ()
...
function SceneSerializer:_collecteProtoEntity ( entity, objMap, protoEntry, namespace, modification, protoInfo )
	...
	tsort ( childrenList, actorSortFunc )
...
local function collectGroup ( group, actorList, objMap )
	local childGroupDatas = {}
	local actorIds = {}
	
	for childGroup in pairs ( group.childGroups) do
		local childGroupData = collectGroup ( childGroup, actorList, objMap )
...
function SceneSerializer:_serializeProto ( entity, id )
	local protoData = self:serializeSingleActor ( entity, 'keepProto' )
	...
	local info = {
		id         = id,
		name       = entity:getName (),
		timestamp  = entity.PROTO_TIMESTAMP or 0,
		serialized = output
	}
	return info
end
...
function SceneSerializer:_flushObjectMap ( objMap )
	...
	for obj, id in pairs ( newObjects ) do
		map[ id ] = SerializerModule._serializeObject ( obj, objMap )
	end
	--dep member pass
	while true do
		local newObjects = objMap:flush ()
...
function SceneSerializer:serializeEntities ( actorList, output, objMap, keepProto )
	output = output or {}
	...
	if keepProto then --proto support
		--collect entity	
		local protoInfo = {}
		for i, e in ipairs ( actorList ) do
		...
		for id, info in pairs ( protoInfo ) do
			...
			local overridedData = {}
			local extraData = {}
			collectOverrideActorData ( objMap, obj, overridedData, extraData )
		...
		--collect entity	
		for i, e in ipairs ( actorList ) do
			local data = self:collectActor ( e, objMap )
			if data then tinsert ( entityDatas, data ) end
		end
...
function SceneSerializer:serializeSingleEntity ( entity, keepProto )
	...
	output[ '__VERSION' ] = SceneAssetModule._SERIALIZER_VERSION
	return output, objMap
end
...
function SceneSerializer:serializeScene ( scene, keepProto )
	...
	indexData[ '__VERSION' ] = SceneAssetModule._SERIALIZER_VERSION
...
function SceneSerializer:serializeRootGroup ( rootGroup, keepProto )
	local output = {}
	local objMap = SerializerModule.SerializeObjectMap ()
	...
	local actorList = {}
	output[ 'groups' ] = {
		collectGroup ( rootGroup, actorList, objMap )
	}
	tsort ( actorList, actorSortFunc )
	self:serializeEntities ( actorList, output, objMap, keepProto )
	...
	output[ 'asset_dependency' ] = SceneAssetWalkerModule.collectGroupAssetDependency ( rootGroup )
...
function SceneSerializer:serializeSceneToFile ( scene, path, keepProto )
	...
	for i, filename in ipairs ( files ) do
		if filename:endwith ( SceneAssetModule._SCENE_GROUP_EXTENSION ) then
			MOAIFileSystem.deleteFile ( path .. '/' .. filename )
		end
	end

	local indexData = data[ 'index' ]
	local indexDataPath = path .. '/' .. SceneAssetModule._SCENE_INDEX_NAME
	...
	for id, groupData in pairs ( groupListData ) do
		local fileName = id .. SceneAssetModule._SCENE_GROUP_EXTENSION
...
local SceneDeserializer = CLASS: SceneDeserializer ()
...
function SceneDeserializer:deserializeGroup ( scene, parentGroup, data, objMap )
	...
	for _, actorId in ipairs ( data[ 'entities' ] ) do
		entity = objMap[ actorId ][ 1 ]
		entity._entityGroup = group
	end
...
function SceneDeserializer:_prepareEntitiesObjMap ( data, objMap )
	...
	-- end
	SerializerModule._prepareObjectMap ( map, objMap )
	return objMap
end

function SceneDeserializer:_deserializeEntitiesData ( data, objMap, scene )
	local map = data[ 'map' ]
	SerializerModule._deserializeObjectMapData ( objMap )
	for id, objData in pairs ( map ) do
	...
	--insetEntity
	for i, adata in ipairs ( data[ 'entities' ] ) do
		self:insertEntity ( scene, nil, adata, objMap )
	end
...
--------------------------------------------------------------------
function makeEntityCopyData ( entity )
	local data, objMap = serializeEntity ( entity, 'keepProto' )
	local rootId = data[ 'entities' ][ 1 ][ 'id' ]
	local newGuids = {}
...
function makeEntityCloneData ( entity, idGenerator )
	local copyData = makeEntityCopyData ( entity )
	return makeEntityPasteData ( copyData, idGenerator )
end

function copyAndPasteEntity ( entity, idGenerator )
	local pasteData = makeEntityCloneData ( entity, idGenerator )
	local created = candy.deserializeEntity ( pasteData )
	return created
end
...
function makeComponentCopyData ( com )
	local data = candy.serialize ( com )
	return data
end
...
--------------------------------------------------------------------
function serializeEntity ( entity, keepProto )
	local data, objMap = SceneSerializer ():serializeSingleEntity ( entity, keepProto )
	return data, objMap
end
...
local function sceneLoader ( node, option )
	local data = node.cached and node.cached.data
		...
		if dep then
			for assetPath, tag in pairs ( dep ) do
				if tag == 'preload' and canPreload ( assetPath ) then
					candy.loadAsset ( assetPath, { preload = true } )
				end
			end
		end
...
local function runtimeLoadScene ( path, option )
	local node = AssetLibraryModule.registerAssetNode ( path, {
			type = 'scene',
			groupType = 'package',
			objectFiles = { def = path } 
		}
	)
	local texture = getTextureLibrary ():affirmTexture ( path )
	return node, texture
end


AssetLibraryModule.registerAssetLoader ( 'scene', sceneLoader, sceneUnloader )

AssetLibraryModule.registerRuntimeAssetLoader ( 'scene', runtimeLoadScene )
AssetLibraryModule.registerRuntimeAssetTypeHinter ( 'scene', 'scene' )

SceneAssetModule.SceneSerializer = SceneSerializer
SceneAssetModule.SceneDeserializer = SceneDeserializer
SceneAssetModule.serializeScene = serializeScene
SceneAssetModule.deserializeScene = deserializeScene
SceneAssetModule.serializeSceneToFile = serializeSceneToFile
SceneAssetModule.makeEntityCopyData = makeEntityCopyData
SceneAssetModule.makeEntityPasteData = makeEntityPasteData
SceneAssetModule.makeEntityCloneData = makeEntityCloneData
SceneAssetModule.copyAndPasteEntity = copyAndPasteEntity
SceneAssetModule.makeEntityGroupPasteData = makeEntityGroupPasteData
SceneAssetModule.makeComponentCopyData = makeComponentCopyData
SceneAssetModule.makeComponentPasteData = makeComponentPasteData
SceneAssetModule.serializeEntity = serializeEntity
SceneAssetModule.deserializeEntity = deserializeEntity
SceneAssetModule.setSceneGroupFilter = setSceneGroupFilter
SceneAssetModule.matchSceneGroupFilter = matchSceneGroupFilter
SceneAssetModule.loadSceneDataFromPath = loadSceneDataFromPath

return SceneAssetModule