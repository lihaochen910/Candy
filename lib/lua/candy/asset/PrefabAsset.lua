...
function Prefab:createInstance ()
	...
	if not data or not next ( data.entities ) then
		_stat ( "loading empty prefab" )
		instance = EntityModule.Entity ()
	else
...
local function PrefabLoader ( node )
...
AssetLibraryModule.registerAssetLoader ( "prefab", PrefabLoader )
