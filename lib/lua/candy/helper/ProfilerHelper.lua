...
function startProfiler ()
	if not ProFi then
		ProFi = require 'candy.3rdparty.ProFi'
	end
	ProFi:reset ()
	ProFi:start ()
end
...
function runProfiler ( duration, outputPath )
	...
	laterCall (
		...
		function ()
			...
			candystopProfiler ( outputPath or 'profiler.log' )
		end
	)
end

ProfilerHelperModule.startProfiler = startProfiler
ProfilerHelperModule.stopProfiler = stopProfiler
ProfilerHelperModule.runProfiler = runProfiler

return ProfilerHelperModule