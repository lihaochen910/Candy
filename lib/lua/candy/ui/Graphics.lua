...
---@class Graphics : DrawScript
local Graphics = CLASS: Graphics ( DrawScript )
...
---
-- The constructor.
function Graphics:__init ( width, height )
    self.commands = {}
end
...
function Graphics:onDraw ( index, xOff, yOff, xFlip, yFlip )
    ...
    MOAIDraw.setPenColor ( self:getColor () )
    MOAIDraw.setPenWidth ( 1 )
    -- In MOAI SDK V1.7 is not supported.
    if MOAIDraw.setPointSize then
        MOAIDraw.setPointSize ( 1 )
    end
...
function Graphics:setPenColor ( r, g, b, a )
    ...
    local command = function ( self )
        ...
        blue = b * blue
        alpha = a * alpha
        MOAIDraw.setPenColor ( red, green, blue, alpha )
    end
    table.insert ( self.commands, command )
    return self
end
...
function Graphics:setPointSize ( size )
    ...
    if MOAIDraw.setPointSize then
        local command = function ( self )
            MOAIDraw.setPointSize ( size )
        end
        table.insert ( self.commands, command )
    else
        _warn ( "MOAIDraw.setPointSize is not supported from MOAI SDK V1.7." )
    end
    return self
end
