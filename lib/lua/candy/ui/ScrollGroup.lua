...
local Graphics = require 'candy.ui.Graphics'
local Event = require 'candy.common.Event'
local UIGroup = require 'candy.ui.UIGroup'

---@class ScrollGroup : UIGroup
local ScrollGroup = CLASS: ScrollGroup ( UIGroup )
...
function ScrollGroup:_createContentGroup ()
    ...
    self._contentGroup:attach ( self._contentBackground )
...
function ScrollGroup:_createScrollBar ()
    self._verticalScrollBar = PatchSprite ( self:getStyle ( ScrollGroup.STYLE_VERTICAL_SCROLL_BAR_TEXTURE ) )
    self._horizontalScrollBar = PatchSprite ( self:getStyle ( ScrollGroup.STYLE_HORIZONTAL_SCROLL_BAR_TEXTURE ) )
...
function ScrollGroup:_startLooper ()
    if not self._looper then
        self._looper = game.callLoop ( self.onEnterFrame, self )
    end
end
...
function ScrollGroup:dispatchTouchCancelEvent ()
    ...
    local e = Event ( UIEvent.touch_cancel )
