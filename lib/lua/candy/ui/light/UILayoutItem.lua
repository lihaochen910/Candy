...
local UILayoutItem = CLASS: UILayoutItem ( Component )
	:MODEL {
...
function UILayoutItem:onEditorInit ()
	...
	if ent:isInstance ( candy.UIWidget ) then
...
function UILayoutItem:getParentLayout ()
	...
	if not isInstance ( widget, candy.UIWidget ) then
		return false
	end
...
function UILayoutItem:sync ()
	...
	if not ent or not ent:isInstance ( UIWidget ) then
		return
	end
...
ComponentModule.registerComponent ( "UILayoutItem", UILayoutItem )
