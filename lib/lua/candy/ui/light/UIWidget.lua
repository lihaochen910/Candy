...
local DataSheetDictAccessor = DataSheetAssetModule.DataSheetDictAccessor
local UILayoutModule = require 'candy.ui.light.UILayout'
local UILayoutEntry = UILayoutModule.UILayoutEntry
local UIWidgetFXModule = require 'candy.ui.light.UIWidgetFX'
local UIWidgetFXHolder = UIWidgetFXModule.UIWidgetFXHolder
...
local UIWidgetBase = CLASS: UIWidgetBase ( Entity )
	:MODEL {
...
local newUIProp = MOAIGraphicsProp.new
...
function UIWidgetBase:updateStringTable ()
	local stringTableDict = self.stringTable and candy.loadAsset ( self.stringTable )
...
function UIWidgetBase:_updateTranslation ( forced )
	...
	for child in pairs ( self:getChildren () ) do
		if child.FLAG_UI_WIDGET and  ( forced or not child.stringTableDict ) then
...
function UIWidgetBase:setLocalStyleSheet ( path )
	...
	self.localStyleSheet = path and candy.loadAsset ( path )
...
function UIWidgetBase:updateRenderOrder ()
...
local UIWidget = CLASS: UIWidget ( UIWidgetBase )
	:MODEL {
...
function UIWidget:__init ()
	self.styleAcc = candy.UIStyleAccessor ( self )
	self.w = 100
	self.h = 100
...
function UIWidget:getLayoutItem ()
	return self:com ( candy.UILayoutItem )
end
...
function UIWidget:initRenderer ()
	if self.rendererInited then
		return
	end

	self.rendererInited = true
	local rendererClassName = self:getStyleAcc ():get ( "renderer", false )
	local clas = _getRendererClass ( rendererClassName ) or self:getDefaultRendererClass ()
	if clas then
...
function UIWidget:setModal ( modal )
	...
	if self._parentView then
		...
		elseif self._parentView:getModalWidget () == self then
...
function UIWidget:isModalChild ()
	if not self._parentView then
		return false
	end

	local modalWidget = self._parentView:getModalWidget ()

	if not modalWidget then
		return true
	end

	if modalWidget == self then
		return true
	end
...
function UIWidget:updateVisual ()
	...
	if margin then
		self:setMargin ( unpack ( margin ) )
	end

	local spacing = style:getBox ( "spacing" )
	if spacing then
		self:setSpacing ( unpack ( spacing ) )
	end
...
function UIWidget:inside ( x, y, z, pad )
	...
	if pad then
		return bx0 <= x + pad and bx1 >= x - pad and by0 <= y + pad and by1 >= y - pad
	else
		return bx0 <= x and x <= bx1 and by0 <= y and y <= by1
	end
end
...
function UIWidget:setSize ( w, h, updateLayout, updateStyle )
	...
	self:updateClippingRect ()
	self:postEvent ( UIEvent ( UIEvent.RESIZE, {
		size = { w, h }
	} ) )
end
...
function UIWidget:getInnerRect ()
	local sl, st, sr, sb = self:getSpacing ()
	local x0, y0, x1, y1 = self:getLocalRect ()
...
local function _findLayoutParent ( widget )
	...
	while p do
		...
		if not p1 or not p1.layout then
			break
		end
...
function UIWidget:getChildLayoutEntries ()
	...
	for i, widget in ipairs ( self.childWidgets ) do
		if not widget.layoutDisabled and not widget.FLAG_INTERNAL and widget:isLocalVisible () then
...
function UIWidget:setSpacing ( l, t, r, b )
	if not l then
		...
		if _eqBox ( self.overridedMetrics.spacing, l, t, r, b ) then
			return
		end
...
function UIWidget:setMargin ( l, t, r, b )
	if not l then
		...
		if _eqBox ( self.overridedMetrics.margin, l, t, r, b ) then
			return
		end
...
function UIWidget:affirmLayoutItem ()
	local layoutItem = self:com ( candy.UILayoutItem )
...
function UIWidget:_toolAffirmLayoutItem ()
	if not self:com ( candy.UILayoutItem ) then
...
function UIWidget:raise ()
	self:show ()
	if not self:hasFocus () and not self:hasChildFocus () then
		self:setFocus ()
	end
end
...
UIWidgetModule.UIWidgetBase = UIWidgetBase
UIWidgetModule.UIWidget = UIWidget

return UIWidgetModule