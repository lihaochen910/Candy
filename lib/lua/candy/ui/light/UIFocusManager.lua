...
local UIFocusManager = CLASS: UIFocusManager ()
	:MODEL {}
...
function UIFocusManager:activate ()
	if _activeFocusManager == self then
		return
	end
...
local function canFocus ( widget )
	...
	if not widget:isInteractive () then
		return false
	end
...
function UIFocusManager:getNextFocus ( widget, dir, wrap )
end
...
function UIFocusManager:findDirectionalFocusConnection ( group, widget, dir )
	local minDistSameLine, resultSameLine, minDistOtherLine, resultOtherLine = nil
	...
	if dir == "n" then
		for w in pairs ( group ) do
			if w ~= widget and canFocus ( w ) then
				...
				local x =  ( l + r ) / 2
				local y =  ( b + t ) / 2
				local pass = b0 < t and t0 < b

				if pass then

					local sameLine = r >= l0 and r0 >= l
					if sameLine then
