...
local UIScrollArea = require 'candy.ui.light.widgets.UIScrollArea'
local EntityModule = require 'candy.Entity'

-- module
local UIListViewModule = {}
...
local UIListItem = CLASS: UIListItem ( UIButton )
	:MODEL {}
...
function UIListItem:isInView ( padding )
	...
	return px0 <= x0 + padding and py0 <= y0 + padding and px1 >= x1 - padding and py1 >= y1 - padding
end
...
function UIListItem:procEvent ( ev )
	...
	local t = ev.type
	if t == UIEvent.POINTER_DCLICK and self:isSelected () then
...
UIListView = CLASS: UIListView ( UIWidget )
	:MODEL {
...
EntityModule.registerEntity ( "UIListView", UIListView )
...
function UIListView:activateItem ( item )
	if self.selection ~= item and self:selectItem ( item ) == false then
		return false
	end
...
function UIListView:calcGridId ( x, y )
	...
	if x < 0 or col <= x then
		return false
	end

	if y < 0 or row <= y then
		return false
	end
...
function UIListView:procInputCommand ( cmdData )
	...
	elseif cmd == "confirm" and self.selection then
...
function UIListView:onInputCommandNavigate ( dir, down )
	...
	if not selection then
		...
		if self:isValidGridLoc ( x, y ) then
			...
			if not item1 and dir ~= "left" then
				if dir ~= "right" then
					for xx = 0, x do
		...
		if item1 and item1.selectable and item1:isVisible () then
			...
			if target and target.hasSelectableItem and not target:hasSelectableItem () then
				return
			end
...
UIListViewModule.UIListItem = UIListItem
UIListViewModule.UIListView = UIListView

return UIListViewModule