-- import
local UIFrame = require 'candy.ui.light.widgets.UIFrame'
local EntityModule = require 'candy.Entity'

---@class UIScrollArea : UIFrame
local UIScrollArea = CLASS: UIScrollArea ( UIFrame )
	:MODEL {
...
EntityModule.registerEntity ( "UIScrollArea", UIScrollArea )
...
function UIScrollArea:onLoad ()
	...
	self:connect ( self.scrollBarH.valueChanged, "syncScrollBarH" )
	self:connect ( self.scrollBarV.valueChanged, "syncScrollBarV" )
...
function UIScrollArea:actionSmoothScroll ()
	...
	while true do
		...
		local tx = self.scrollX
		local ty = self.scrollY
