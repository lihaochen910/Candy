...
--------------------------------------------------------------------
---@class UILayoutEntry
local UILayoutEntry = CLASS: UILayoutEntry ()
	:MODEL {}
...
function UILayoutEntry:setFrameSize ( width, height )
	...
	if self.minWidth <= 0 and self.minHeight <= 0 then
...
function UILayoutEntry:apply ()
	...
	local fh = self.frameHeight
	local targetWidth = self.targetWidth or 0
	local targetHeight = self.targetHeight or 0
	local x = self.locX or 0
	local y = self.locY or 0
	local offsetX = 0
	local offsetY = 0
...
function UILayoutEntry:fitMinSize ( w, h )
	local policyH = self.policyH
	local policyV = self.policyV
	local w1 = w
	local h1 = h
...
local UILayout = CLASS: UILayout ( Component )
	:MODEL {
...
function UILayout:getMaxInnerSize ()
	...
	local marginL, marginT, marginR, marginB = self:calcMargin ()
	w = w < 0 and -1 or w - marginL - marginR
	h = h < 0 and -1 or h - marginT - marginB
	return w, h
end
...
function UILayout:onAttach ( ent )
	if not ent:isInstance ( candy.UIWidget ) then
...
function UILayout:_toolAddChildLayoutItems ()
	if not self:getEntity ():isInstance ( candy.UIWidget ) then
		return
	end
...
UILayoutModule.UILayoutEntry = UILayoutEntry
UILayoutModule.UILayout = UILayout

return UILayoutModule