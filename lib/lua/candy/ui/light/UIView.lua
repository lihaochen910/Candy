...
local UIFocusManager = require 'candy.ui.light.UIFocusManager'
...
local UIFocusCursor = require 'candy.ui.light.UIFocusCursor'
local UIPointer = require 'candy.ui.light.UIPointer'
local InputDeviceModule = require 'candy.input.InputDevice'
local TouchState = InputDeviceModule.TouchState
local Camera = require 'candy.gfx.Camera'
local BufferedTable = require 'candy.tools.BufferedTable'
...
local UIView = CLASS: UIView ( UIWidgetBase )
	:MODEL {
		"----";
...
EntityModule.registerEntity ( "UIView", UIView )
...
function UIView:setInputCommandMapping ( mapping )
	if self.inputMapping == mapping then
		return
	end
...
function UIView:setInputCategory ( category )
	if self.inputCategory == category then
		return
	end
...
function UIView:setContextData ( data )
...
function UIView:updateFocusCursor ()
	if not getUIOption ( "focus_cursor", true ) then
		return
	end

	if not self.focusCursor then
		return
	end
	...
	for k = count, 1, -1 do
		...
		if child:isInteractive () then
			...
			elseif inside then
				...
				if not result and child:isTrackingPointer () then
					result = child
				end
...
function UIView:onKeyChar ( char )
	...
	local ev = UIEvent ( UIEvent.TEXT_INPUT, {
		text = char,
		modifiers = getModifierKeyStates ()
	} )
...
function UIView:onJoyAxisMove ( joy, axis, value )
	...
	local ev = UIEvent ( UIEvent.JOYSTICK_AXIS_MOVE, {
		axis = axis,
		value = value,
		joystick = joy
	} )
