...
local TwoKeyTable = require 'candy.tools.MultiKeyTable'
...
local function getWidgetClassID ( clas )
	...
	if not id then
		...
		while true do
			...
			if c == UIWidgetBase or not c then
				break
			end
...
_G._getWidgetClassAbbr = getWidgetClassAbbr
_G._getWidgetFeatureAbbr = getWidgetFeatureAbbr
_G._getWidgetClassID = getWidgetClassID

---@class UIStyleAccessor
local UIStyleAccessor = CLASS: UIStyleAccessor ()
	:MODEL {}
...
function UIStyleAccessor:getQueryList ()
	local list = self.queryList
	local fullQuery = self.fullQuery
	local hasFeatureQuery = self.hasFeatureQuery

	if list then
		return list, fullQuery
	end
...
function UIStyleAccessor:buildQueryList ()
	...
	if parent and not parent:isRootWidget () then
		...
		if pacc.hasFeatureQuery then
			hasFeatureQuery = true
		end
	else
		queryList = {
			localQuery
...
function UIStyleAccessor:getAsset ( key, default )
	...
	if tt == "table" then
		if data.tag == "asset" then
			return data.asset
		end
		if data.tag == "object" then
			return data.object
		end
...
function UIStyleAccessor:getBox ( key, default )
	local data = self.cachedData[ key ]
