...
--------------------------------------------------------------------
-- UIGraphicsPropCursor
--------------------------------------------------------------------
---@class UIGraphicsPropCursor : UICursor
local UIGraphicsPropCursor = CLASS: UIGraphicsPropCursor ( UICursor )
...
--------------------------------------------------------------------
-- UITexturePlaneCursor
--------------------------------------------------------------------
---@class UITexturePlaneCursor : UIGraphicsPropCursor
local UITexturePlaneCursor = CLASS: UITexturePlaneCursor ( UIGraphicsPropCursor )

function UITexturePlaneCursor:__init ( texture, w, h, ox, oy )
	if type ( texture ) == "string" then
		texture = candy.loadAsset ( texture )
	end
...
local UIDefaultCursor = CLASS: UIDefaultCursor ( UIGraphicsPropCursor )

function UIDefaultCursor:onLoad ()
	local pngData = require ( "candy.ui.light.default_cursor_png" )
	local img = MOAIHelpersModule.loadImageBase64 ( nil, pngData )
...
local UICursorSimple = CLASS: UICursorSimple ( UICursor )
...
local UICursorManager = CLASS: UICursorManager ( GlobalManager ):MODEL {}
...
function UICursorManager:cursorToWnd ( x, y )
	local wx = x * self.sx + self.ox
	local wy = y * self.sy + self.oy
	return wx, wy
end

local clamp = math.clamp
function UICursorManager:updateLocRelative ( x, y, rx, ry )
	local w, h = game:getContentResolution ()
	local ww, hh = game:getDeviceResolution ()
	local ratioX = w / ww
	local ratioY = h / hh
	local margin = self.margin or 0
	local x0 = self.targetX
	local y0 = self.targetY
	local x1 = x0 + rx * ratioX
	local y1 = y0 + ry * ratioY
...
UICursorModule.UICursor = UICursor
UICursorModule.UIGraphicsPropCursor = UIGraphicsPropCursor
UICursorModule.UITexturePlaneCursor = UITexturePlaneCursor
UICursorModule.UIDefaultCursor = UIDefaultCursor
UICursorModule.UICursorSimple = UICursorSimple
UICursorModule.UICursorManager = UICursorManager
UICursorModule.getUICursorManager = getUICursorManager
UICursorModule.setCursor = setCursor
UICursorModule.registerCursor = registerCursor

return UICursorModule