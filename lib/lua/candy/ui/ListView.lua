...
-- <h4>Extends:</h4>
-- <ul>
--   <li><a href="candy.ui.PanelView.html">PanelView</a><l/i>
-- </ul>
--
-- @author Makoto
-- @release V3.0.0
...
-- import
local PanelView = require 'candy.ui.PanelView'
local ListViewLayout = require 'candy.ui.ListViewLayout'

---@class ListView : PanelView
local ListView = CLASS: ListView ( PanelView )
...
ListView.EVENT_ITEM_DOUBLE_CLICK = "itemDoubleClick"
...
function ListView:_initInternal ()
    ...
    self._touchedOldRenderer = nil
    self._touchedOldRendererTime = os.clock ()
end
...
function ListView:_updateItemRenderers ()
    ...
    -- print('_updateItemRenderers', #self._itemRenderers, inspect ( self._itemRenderers))
    ...
    if renderersSize > self:getDataLength () then
        ...
        for i = renderersSize, self:getDataLength () + 1, -1 do
            -- print('remove idx:', i, self._itemRenderers[i])
...
function ListView:_updateItemRenderer ( data, index )
    ...
    if not renderer then
        ...
        -- print('create render idx:', index, self._itemRenderers[index])
    end
...
function ListView:getSelectedIndex ()
    ...
    return item and table.index ( self:getDataSource (), item ) or -1
end
...
function ListView:getSelectedItems ()
    return table.simplecopy ( self._selectedItems )
end
...
function ListView:removeItemAt ( index )
    if index <= self:getDataLength () then
        table.remove ( self._dataSource, index )
        self:invalidateItemRenderers ()
    end
end
...
---
-- Set the event listener that is called when the item double click.
---@param func selected changed event handler
function ListView:setOnItemDoubleClick ( func )
    self:setEventListener ( ListView.EVENT_ITEM_DOUBLE_CLICK, func )
end
...
function ListView:onItemRendererTouchUp ( e )
    ...
    if renderer.isRenderer then
        ...
        local doubleClickTriggered = false

        if self._touchedOldRenderer == renderer and os.clock  () - self._touchedOldRendererTime <= InputMgr.DOUBLE_TAP_INTERVAL then
            self:dispatchEvent ( ListView.EVENT_ITEM_DOUBLE_CLICK, renderer:getData () )
            doubleClickTriggered = true
            -- print("double tap!! interval =", os.clock  () - self._touchedOldRendererTime)
        end

        self._touchedOldRenderer = renderer
        self._touchedOldRendererTime = doubleClickTriggered and 0 or os.clock  ()
