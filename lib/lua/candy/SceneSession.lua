...
---@class SceneSession
local SceneSession = CLASS: SceneSession ()
	:MODEL {}
...
function SceneSession:openSceneByPath ( scnPath, additive, arguments, autostart )
	...
	local scn, node = candy.loadAsset (
		scnPath, 
		{ 
			scene = scene,
	...
	SignalModule.emitGlobalSignal ( 'scene.open', scn, arguments )
	if self.main then
		SignalModule.emitGlobalSignal ( 'mainscene.open', scn, arguments )
	end
	...
	SignalModule.emitGlobalSignal ( 'scene.start', scn, arguments )
	if self.main then
		SignalModule.emitGlobalSignal ( 'mainscene.start', scn, arguments )
	end
	return scn
end
...
function SceneSession:scheduleOpenSceneByPath ( scnPath, additive, arguments, autostart )
	...
	SignalModule.emitGlobalSignal ( 'scene.schedule_open', self.pendingLoading )
	if self.main then
		SignalModule.emitGlobalSignal ( 'mainscene.schedule_open', self.pendingLoading )
	end
end
