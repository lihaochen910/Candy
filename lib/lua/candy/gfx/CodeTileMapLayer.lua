...
-- module
local CodeTileMapLayerModule = {}

---@class CodeTileGrid : TileMapGrid
local CodeTileGrid = CLASS: CodeTileGrid ( TileMapGrid )
	:MODEL {}
...
function CodeTileGrid:getTile ( x, y )
...
function CodeTileGrid:loadTiles ( data )
	...
	if notMatch then
		...
		for k, id1 in pairs ( nameToId ) do
			...
			if id2 == 0 then
				_warn ( "symbol not found in tileset" )
			end
...
local CodeTileMapLayer = CLASS: CodeTileMapLayer ( TileMapLayer )
	:MODEL {}
...
function CodeTileMapLayer:onResize ( x0, y0, x1, y1 )
	...
	local h = y1 - y0
	local tw, th = self:getTileSize ()
	local sw = w * subdivision
	local sh = h * subdivision
	local sx = x0 * subdivision
	local sy = y0 * subdivision
...
CodeTileMapLayerModule.CodeTileGrid = CodeTileGrid
CodeTileMapLayerModule.CodeTileMapLayer = CodeTileMapLayer

return CodeTileMapLayerModule