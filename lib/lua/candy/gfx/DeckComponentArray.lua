...
local DeckComponentArray = CLASS: DeckComponentArray ( GraphicsPropComponent )
	:MODEL {
		Field("deck"):asset("deck2d\\..*"):getset("Deck"),
		"----",
		Field("gridSize"):type("vec3"):getset("GridSize"):meta({
			decimals = 0
		}),
...
ComponentModule.registerComponent ( "DeckComponentArray", DeckComponentArray )
...
function DeckComponentArray:updateLayout ()
	...
	if self.propBound then
		...
		local h = y1 - y0
		local d = z1 - z0
		local bx0, by0, bz0, bx1, by1, bz1 = nil
		...
		self:getMoaiProp ():setBounds ( bx0, by0, bz0, bx1, by1, bz1 )
	else
