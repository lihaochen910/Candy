...
--------------------------------------------------------------------
---@class TileMapGrid
local TileMapGrid = CLASS: TileMapGrid ()
	:MODEL {}
...
function TileMapGrid:isValidCoord ( x, y )
	...
	if w < x then return false end
	if h < y then return false end
...
function TileMapGrid:getTile ( x, y )
...
function TileMapGrid:saveTiles ()
	local data = {
		tiles = MOAIHelpersModule.saveMOAIGridTiles ( self.grid )
	}
	return data
end
...
--------------------------------------------------------------------
---@class TileMapLayer
local TileMapLayer = CLASS: TileMapLayer ()
	:MODEL {
		Field "name" :string();
		Field "tag" :string();
...
function TileMapLayer:__collect_dependency ( collected, collector )
	if self.tilesetPath then
		collector ( self, false, self.tilesetPath, collected )
	end

	if self.materialPath then
		collector ( self, false, self.materialPath, collected )
	end
end
...
function TileMapLayer:saveData ()
	local data = {
		...
		tiles = self:getGrid ():saveTiles (),
		tileset = self:getTilesetPath (),
		order = self.order,
		subdivision = self.subdivision,
		material = self.materialPath,
		offset = self.offset
	}
...
function TileMapLayer:onDrawGridLine ()
	...
	local x0 = 0
	local x1 = width * tileWidth
	local y0 = 0
	local y1 = height * tileHeight
...
function TileMapLayer:updateMaterial ()
	...
	materialObject = materialObject or self.parentMap:getMaterialObject ()
...
--------------------------------------------------------------------
---@class TileMapParam
local TileMapParam = CLASS: TileMapParam ()
	:MODEL {
		Field "defaultTileset" :asset("tileset;named_tileset");
...
local TileMapResizeParam = CLASS: TileMapResizeParam ()
	:MODEL {
...
local TileMap = CLASS: TileMap ( RenderComponent )
	:MODEL {
		...
		Field "size" :type("vec2"):getset("Size"):readonly():meta({
			decimals = 0
		});
		Field "tileSize" :type("vec2"):getset("TileSize"):readonly():meta({
			decimals = 0
		});
		"----";
...
function TileMap:loadPendingMapData ()
	if not self.pendingMapData then
		return
	end
...
function TileMap:findLayerByTileset ( path )
	for i, layer in ipairs ( self.layers ) do
		if layer:getTilesetPath () == path then
			return layer
		end
	end
...
function TileMap:markDataModified ()
	markProtoInstanceFieldsOverrided ( self, "serializedData" )
end


TileMapModule.TileMapGrid = TileMapGrid
TileMapModule.TileMapLayer = TileMapLayer
TileMapModule.TileMapParam = TileMapParam
TileMapModule.TileMapResizeParam = TileMapResizeParam
