...
stateCollectorMT = {
	...
	__newindex = function  ( t, action, func )
		...
		if action ~= "jumpin" and action ~= "jumpout" and action ~= "step" and action ~= "enter" and action ~= "exit" and action ~= "coroutine" then
			return error ( string.format ( "unsupported state action: %s:%s", name, action ) )
		end
...
local FSMController = CLASS: FSMController ( Behaviour )
	:MODEL {
...
function FSMController:__init ()
	...
	function self.msgBoxListener ( msg, data, source )
		if msgFilter and msgFilter ( msg, data, source ) == false then
			return
		end
	...
	function self._msgFilterSetter ( f )
		msgFilter = f
	end
...
ComponentModule.registerComponent ( "FSMController", FSMController )
