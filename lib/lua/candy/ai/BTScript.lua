...
function ParseContextProto:addLineChild ()
	local node = {
		...
		line = self.currentLineNo,
		parent = self.currentLineHead
	}
...
function ParseContextProto:matchIndent ( indent )
	if self.currentIndent < indent then
	...
	while n do
		if n.indent == indent and n.parent.indent < indent then
			found = n
			break
		end
...
function ParseContextProto:parse_sub_def ( content, pos )
	...
	if not self:isCurrentNodeRoot () then
		self:setErrorInfo ( string.format ( "sub tree should be defined at ROOT pos: %s", match ) )
...
function ParseContextProto:parseLine ( lineNo, l )
	...
	if self.decorateState == "decorating" and self.currentParent ~= self.currentDecorationNode then
...
local function stripNode ( n )
	...
	for i, child in ipairs ( n.children ) do
		if not child.commented and not not child.type then
...
function loadBTScript ( src, name )
	...
	local outputNode = try ( function  ()
		return context:parseSource ( src )
	end )
...
AssetLibraryModule.registerAssetLoader ( "bt_script", BTScriptLoader )
