...
local PathFinderManager = CLASS: PathFinderManager ( GlobalManager )
	:MODEL {
...
function PathGraph:createPathFinderRequest ( owner, x0, y0, z0, x1, y1, z1, context )
	...
	if not request:init ( x0, y0, z0, x1, y1, z1 ) then
		return false
	end
...
local PathFinderRequest = CLASS: PathFinderRequest ()
...
function PathFinderRequest:init ( x0, y0, z0, x1, y1, z1 )
	...
	if not node0 or not node1 then
		return false
	end
...
local PathFinder = CLASS: PathFinder ( Component )
	:MODEL {
		Field ( "targetGraphID" ):string ()
	}

ComponentModule.registerComponent ( "PathFinder", PathFinder )
...
PathFinderModule.getPathFinderManager = getPathFinderManager
PathFinderModule.PathFinderManager = PathFinderManager
PathFinderModule.PathGraph = PathGraph
PathFinderModule.PathFinder = PathFinder
