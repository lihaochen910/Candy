...
--------------------------------------------------------------------
-- import
local UpdateListener = require 'candy.common.UpdateListener'
local ComponentModule = require 'candy.Component'

-- module
local BTControllerModule = {}
...
function BTContext:requestAction ( actionNode )
	...
	actionClas = actionClas or ActionRegistry[ name ]
	local action = nil
...
function BTContext:updateRunningNodes ( dt )
	...
	for i = 1, count do
		...
		if node and node:update ( self, dt, false ) == "complete" then
			return "complete"
		end
	end
...
function BTActionNode:execute ( context )
	if self:pre_execute ( context ) == false then
		return false
	end
...
function BTActionNode:update ( context, dt, fromExecute )
	...
	assert ( res == "ok" or res == "fail" or res == "running", string.format ( "invalid action return value inside %s: %s", self.actionName, tostring ( res ) ) )
...
local BTLoggingNode = CLASS: BTLoggingNode ( BTNode )
	:MODEL {}
...
local BTMsgSendingNode = CLASS: BTMsgSendingNode ( BTNode )
	:MODEL {}
...
local BTConditionNot = CLASS: BTConditionNot ( BTCondition )
...
local BTCompositedNode = CLASS: BTCompositedNode ( BTNode )
...
function BTCompositedNode:getType ()
...
function BTCompositedNode:validate ( context )
	for i, nn in ipairs ( self.children ) do
		if not nn:validate ( context ) then
			return false
		end
	end
...
function BTCompositedNode:postLoad ( context )
	for i, nn in ipairs ( self.children ) do
		if not nn:postLoad ( context ) then
			return false
		end
	end
...
function BTCompositedNode:removeNode ( node )
...
local BTPrioritySelector = CLASS: BTPrioritySelector ( BTCompositedNode )
...
local BTRootNode = CLASS: BTRootNode ( BTPrioritySelector )
...
local BTSequenceSelector = CLASS: BTSequenceSelector ( BTCompositedNode )
...
local BTRandomSelector = CLASS: BTRandomSelector ( BTCompositedNode )
...
local BTShuffledSequenceSelector = CLASS: BTShuffledSequenceSelector ( BTCompositedNode )
...
local BTConcurrentAndSelector = CLASS: BTConcurrentAndSelector ( BTCompositedNode )
...
function BTConcurrentAndSelector:checkResult ( context )
	...
	if failCount > 0 then
		...
		return self:returnUpLevel ( "fail", context )
	elseif self.childrenCount <= okCount then
		env.okCount = 0
		env.failCount = 0
		env.firstRun = true
		return self:returnUpLevel ( "ok", context )
...
local BTConcurrentOrSelector = CLASS: BTConcurrentOrSelector ( BTCompositedNode )
...
function BTConcurrentOrSelector:checkResult ( context )
	...
	if self.childrenCount <= failCount then
		env.okCount = 0
		env.failCount = 0
		env.firstRun = true
		context:removeRunningChildNodes ( self )
		return self:returnUpLevel ( "fail", context )
	elseif self.childrenCount <= okCount + failCount then
		env.okCount = 0
		env.failCount = 0
		env.firstRun = true
		return self:returnUpLevel ( "ok", context )
...
local BTConcurrentEitherSelector = CLASS: BTConcurrentEitherSelector ( BTCompositedNode )
...
function BTConcurrentEitherSelector:checkResult ( context )
	...
	if okCount > 0 then
		...
		context:removeRunningChildNodes ( self )
		return self:returnUpLevel ( "ok", context )
	elseif self.childrenCount <= failCount then
		env.okCount = 0
		env.failCount = 0
		env.firstRun = true
		return self:returnUpLevel ( "fail", context )
...
local BTDecoratorNot = CLASS: BTDecoratorNot ( BTDecorator )
...
local BTDecoratorAlwaysOK = CLASS: BTDecoratorAlwaysOK ( BTDecorator )
...
local BTDecoratorAlwaysFail = CLASS: BTDecoratorAlwaysFail ( BTDecorator )
...
local BTDecoratorAlwaysIgnore = CLASS: BTDecoratorAlwaysIgnore ( BTDecorator )
...
local BTDecoratorRepeatUntil = CLASS: BTDecoratorRepeatUntil ( BTDecorator )
...
local BTDecoratorRepeatFor = CLASS: BTDecoratorRepeatFor ( BTDecorator )
...
local BTDecoratorRepeatWhile = CLASS: BTDecoratorRepeatWhile ( BTDecorator )
...
local BTDecoratorRepeatForever = CLASS: BTDecoratorRepeatForever ( BTDecorator )
...
local BTDecoratorWeight = CLASS: BTDecoratorWeight ( BTDecorator )
...
local BTDecoratorProb = CLASS: BTDecoratorProb ( BTDecorator )
...
local BTController = CLASS: BTController ( UpdateListener )
	:MODEL {
...
ComponentModule.registerComponent ( "BTController", BTController )
...
function BTController:setScheme ( schemePath )
	...
	self.tree = candy.loadAsset ( schemePath )
	self:resetEvaluate ()
end
...
BTControllerModule.BTAction = BTAction
BTControllerModule.BTContext = BTContext
BTControllerModule.BehaviorTree = BehaviorTree
BTControllerModule.BTNode = BTNode
BTControllerModule.BTActionNode = BTActionNode
BTControllerModule.BTLoggingNode = BTLoggingNode
BTControllerModule.BTMsgSendingNode = BTMsgSendingNode
BTControllerModule.BTCondition = BTCondition
BTControllerModule.BTConditionNot = BTConditionNot
BTControllerModule.BTCompositedNode = BTCompositedNode
BTControllerModule.BTPrioritySelector = BTPrioritySelector
BTControllerModule.BTRootNode = BTRootNode
BTControllerModule.BTSequenceSelector = BTSequenceSelector
BTControllerModule.BTRandomSelector = BTRandomSelector
BTControllerModule.BTShuffledSequenceSelector = BTShuffledSequenceSelector
BTControllerModule.BTConcurrentAndSelector = BTConcurrentAndSelector
BTControllerModule.BTConcurrentOrSelector = BTConcurrentOrSelector
BTControllerModule.BTConcurrentEitherSelector = BTConcurrentEitherSelector
BTControllerModule.BTDecorator = BTDecorator
BTControllerModule.BTDecoratorNot = BTDecoratorNot
BTControllerModule.BTDecoratorAlwaysOK = BTDecoratorAlwaysOK
BTControllerModule.BTDecoratorAlwaysFail = BTDecoratorAlwaysFail
BTControllerModule.BTDecoratorAlwaysIgnore = BTDecoratorAlwaysIgnore
BTControllerModule.BTDecoratorRepeatUntil = BTDecoratorRepeatUntil
BTControllerModule.BTDecoratorRepeatFor = BTDecoratorRepeatFor
BTControllerModule.BTDecoratorRepeatWhile = BTDecoratorRepeatWhile
BTControllerModule.BTDecoratorRepeatForever = BTDecoratorRepeatForever
BTControllerModule.BTDecoratorWeight = BTDecoratorWeight
BTControllerModule.BTDecoratorProb = BTDecoratorProb
BTControllerModule.BTController = BTController
BTControllerModule.BehaviorTreeNodeTypes = BehaviorTreeNodeTypes

return BTControllerModule