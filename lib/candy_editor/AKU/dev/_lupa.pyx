...
DEF LUPACACHE = b"__LUPA_CACHE__"
...
cdef class LuaRuntime:
    ...
    * ``unpack_returned_tuples``: should Python tuples be unpacked in Lua?
    ...
    cdef initWithState(self, lua_State* L):
        ...
        self._lock = FastRLock()
        self._pyrefs_in_lua = {}
        self._encoding = _asciiOrNone('UTF-8')
        self._source_encoding = _asciiOrNone('UTF-8') or self._encoding or b'UTF-8'
        self._unpack_returned_tuples = False
        self._register_eval = True
        self._register_builtins = True

        # AKUCreateContext()调用时已经调用过luaL_openlibs, 此处不应该再次调用，否则会导致package.preload被清除
        # lua.luaL_openlibs( L )
