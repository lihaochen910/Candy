...
_INFO_FILE_NAME = '__candy__.json'
_TOOLS = []
...
def loadToolSetting ( path ):
	...
	try:
		data = json.load ( open ( infoFilePath, 'r' ) )
...
def scanTools ( path ):
	from candy_editor_bin import getMainModulePath
...
def startTool ( toolInfo ):
	...
	toolModuleName = 'candy_tool_' + toolInfo[ 'name' ]
...
def printCANDY ():
 ...
 &b--&y     ,_______    &b------------------------------------------------------ 
   ...
   &y / <_         |  &r   |   ||  ||   | |   |                                
   &y/             |  &r   |   |_| ||   | |   |                               
   &y\\,,,,,,,,,,,,/  &r    |_______||___| |___|   &wCandy development environment 
 &b--&y      |   |      &b------------------------------------------------------
 """

	output = str.replace ( output, '&y', Fore.YELLOW + Style.BRIGHT )
	output = str.replace ( output, '&r', Fore.BLUE + Style.NORMAL )
	output = str.replace ( output, '&w', Fore.BLUE + Style.NORMAL )
	output = str.replace ( output, '&b', Fore.BLUE + Style.NORMAL )
	print ( output )
