...
from candy_editor.SceneEditor.SceneTool import SceneTool, SceneToolButton
from candy_editor.moai.MOAIEditCanvas import MOAIEditCanvas
...
class SceneView ( SceneEditorModule ):
	...
	def onLoad ( self ):
		...
		# 		title = 'Scene'
		# 	)
		self.window = self.requestToolWindow ( 'SceneView',
			title = 'Scene',
			area = 'main'
		)
		...
		self.canvas.loadScript ( getModulePath ( 'SceneView.lua', __file__ ) )
		...
		self.gridWidthSpinBox.setButtonSymbols ( QAbstractSpinBox.NoButtons )
		self.toolbar.addWidget ( self.gridWidthSpinBox )

		self.gridHeightSpinBox = QSpinBox ()
		self.gridHeightSpinBox.setRange ( 10, 1000 )
		self.gridHeightSpinBox.valueChanged.connect ( self.onGridHeightChange )
		self.gridHeightSpinBox.setButtonSymbols ( QAbstractSpinBox.NoButtons )
...
class SceneViewCanvas ( MOAIEditCanvas ):
	...
	def dragEnterEvent ( self, ev ):
		...
		pos = ev.pos ()
		accepted = False
		if mimeData.hasFormat ( CANDY_MIME_ASSET_LIST ):
			if self.parentView.onDragStart (
				CANDY_MIME_ASSET_LIST, str (mimeData.data ( CANDY_MIME_ASSET_LIST )),
				pos.x (), pos.y ()):
				accepted = True
		if accepted:
