...
from candy_editor.qt.dialogs                    import alertMessage, requestConfirm, requestString
from candy_editor.qt.controls.GenericTreeWidget import GenericTreeWidget, GenericTreeFilter
from candy_editor.qt.helpers                    import makeBrush, makeFont

from candy_editor.moai.MOAIRuntime              import MOAILuaDelegate
from candy_editor.SceneEditor.SceneEditor       import SceneEditorModule

from candy_editor.qt.controls.SearchView        import requestSearchView, registerSearchEnumerator
...
##----------------------------------------------------------------##
from candy_editor.moai.CandyRuntime import _CANDY, isCandyInstance
##----------------------------------------------------------------##
...
class SceneOutlinerEditor ( SceneEditorModule ):
	...
	def onLoad ( self ):
		...
		self.container = self.requestToolWindow ( 'SceneOutlinerEditor',
		...
		#Components
		self.treeFilter = self.container.addWidget ( GenericTreeFilter (), expanding = False )
		...
		self.delegate.load ( self.getModulePath ( 'SceneOutlinerEditor.lua' ) )
		# self.delegate.load ( self.getApp ().getPath ( 'lib/lua/candy_editor/SceneOutlinerEditor.lua' ) )
	...
	def refreshCreatorMenu ( self ):
		...
		registry = _CANDY.getEntityRegistry ()
		#entity
	...
	def onCopyEntity ( self ):
		...
		for s in self.getSelection ():
			if text == '':
				...
				text = text + '\n' + s.name
		mime.setText ( text )
		mime.setData ( CANDY_MIME_ENTITY_DATA, entityGroupData.encode ('utf-8') )
	...
	def onPasteEntity ( self ):
		clip = QtWidgets.QApplication.clipboard ()
		mime = clip.mimeData ()
		if mime.hasFormat ( CANDY_MIME_ENTITY_DATA ):
			data = mime.data ( CANDY_MIME_ENTITY_DATA )
	...
	def onCopyComponent ( self ):
		...
		for s in self.getSelection ():
			if text == '':
				...
				text = text + '\n' + s.name
		mime.setText ( text )
		mime.setData ( CANDY_MIME_ENTITY_DATA, str (entityGroupData) )
		clip.setMimeData ( mime )
		return True
...
class SceneGraphTreeWidget ( GenericTreeWidget ):
	...
	def getNodeParent ( self, entityNode ): # reimplemnt for target node
		parentNode = _CANDY.Entity.getParentOrGroup ( entityNode )
	...
	def getNodeChildren ( self, node ):
		if isCandyInstance ( node, 'EntityGroup' ):
			...
			for entity in node.entities:
				if ( not entity.parent ) and ( not ( entity.FLAG_EDITOR_OBJECT or entity.FLAG_INTERNAL ) ):
			...
			for entity in node.children:
				if not ( entity.FLAG_EDITOR_OBJECT or entity.FLAG_INTERNAL ):
					output.append ( entity )
			# output = sorted ( output, cmp = _sortEntity )
			return output
	...
	def updateItemContent ( self, item, node, **option ):
		...
		if isCandyInstance ( node, 'EntityGroup' ):
			...
			# if _CANDY.Entity.isLocalEditLocked ( node ):
			# 	item.setIcon ( 2, getIcon ( 'entity_lock' ) )
			# else:
			# 	item.setIcon ( 2, getIcon ( 'entity_nolock' ) )
	...
	def onItemChanged ( self, item, col ):
		self.module.renameEntity (item.node, item.text (1))
...
class SceneGraphTreeItem ( QtWidgets.QTreeWidgetItem ):
	def __lt__ (self, other):
		...
		group0 = isCandyInstance ( node0, 'EntityGroup' )
		group1 = isCandyInstance ( node1, 'EntityGroup' )
		if group0 != group1:
			if tree.sortOrder () == 0:
