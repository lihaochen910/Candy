...
_CANDY_APP_CONFIG_FILE = 'config.json'
...
class EditorApp(object):
	...
	def saveConfig( self ):
		jsonHelper.trySaveJSON( self.config, self.getPath( _CANDY_APP_CONFIG_FILE ), 'project config' )

	def loadConfig( self ):
		loaded = jsonHelper.tryLoadJSON( self.getPath( _CANDY_APP_CONFIG_FILE ) )
		if loaded:
			config = self.config
